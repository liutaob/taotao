...
(function($) {
	...
	function _37(_38, _39) {
		...
		var _3b = $(_38).datagrid("getColumnFields", true).length
				+ (_3a.rownumbers ? 1 : 0);
		...
		function _3d(tr, _3e) {
			$("<tr class=\"treegrid-tr-tree\">"
					+ "<td style=\"border:0px\" colspan=\"" + _3e + "\">"
	...
	function _3f(_40, _41, _42, _43) {
		...
		if (_46) {
			...
			var cc1 = _48.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			var cc2 = _49.next("tr.treegrid-tr-tree").children("td")
					.children("div");
			if (!_43) {
				_46.children = [];
			}
	...
	function _69(_6a, _6b) {
		...
		var _6e = _6d.find("div.datagrid-body tr[node-id=\"" + _6b + "\"]")
				.children("td[field=\"" + _6c.treeField + "\"]");
	...
	function _a6(_a7) {
		...
		function _ab(_ad) {
			...
			if (_ad.length) {
				...
				_b0.find(".tree-join").removeClass("tree-join")
	...
	$.fn.treegrid.methods = {
		...
		reloadFooter : function(jq, _c2) {
			return jq.each(function() {
						...
						if (_c3.showFooter) {
							_c3.view.renderFooter.call(_c3.view, this,
									dc.footer1, true);
							_c3.view.renderFooter.call(_c3.view, this,
									dc.footer2, false);
		...
		update : function(jq, _c8) {
			return jq.each(function() {
						...
						_c9.view.updateRow
								.call(_c9.view, this, _c8.id, _c8.row);
					});
		},
	...
	$.fn.treegrid.parseOptions = function(_ca) {
		return $.extend({}, $.fn.datagrid.parseOptions(_ca), $.parser
						.parseOptions(_ca, ["treeField", {
											animate : "boolean"
										}]));
	};
	var _cb = $.extend({}, $.fn.datagrid.defaults.view, {
		render : function(_cc, _cd, _ce) {
			...
			function _d4(_d5, _d6, _d7) {
				...
				var _d9 = (_d8 ? _d8.children.length : $(_cc)
				...
				for (var i = 0; i < _d7.length; i++) {
					...
					var _de = _d1 + "-" + (_d5 ? 1 : 2) + "-"
							+ row[_cf.idField];
					_da.push("<tr id=\"" + _de + "\" node-id=\""
							+ row[_cf.idField] + "\" " + cls + " " + _dd + ">");
					_da = _da.concat(_d2.renderRow.call(_d2, _cc, _d0, _d5,
							_d6, row));
					_da.push("</tr>");
		...
		renderFooter : function(_df, _e0, _e1) {
			...
			for (var i = 0; i < _e3.length; i++) {
				...
				_e5.push("<tr class=\"datagrid-row\" node-id=\""
						+ row[_e2.idField] + "\">");
			...
			for (var i = 0; i < _e7.length; i++) {
				...
				if (col) {
					...
					if (col.checkbox) {
						...
						cc.push(" name=\"" + _eb + "\" value=\""
								+ (row[_eb] != undefined ? row[_eb] : "")
								+ "\">");
					} else {
						var val = null;
						if (col.formatter) {
		...
		deleteRow : function(_fc, id) {
			...
			if (_fe) {
				if (_fe.children.length == 0) {
					...
					tr.next("tr.treegrid-tr-tree").remove();
					var _ff = tr
							.children("td[field=\"" + _fd.treeField + "\"]")
							.children("div.datagrid-cell");
					_ff.find(".tree-icon").removeClass("tree-folder")
							.addClass("tree-file");
			...
			function _10a(rows) {
				rows.sort(function(r1, r2) {
							...
							for (var i = 0; i < _108.length; i++) {
								var sn = _108[i];
								var so = _109[i];
								var col = $(_107).treegrid("getColumnOption",
										sn);
		...
		finder : {
			getTr : function(_114, id, type, _115) {
				...
				if (_115 == 0) {
					...
					if (type == "body") {
						var tr = $("#" + $.data(_114, "datagrid").rowIdPrefix
								+ "-" + _115 + "-" + id);
						if (!tr.length) {
							tr = (_115 == 1 ? dc.body1 : dc.body2)
									.find("tr[node-id=\"" + id + "\"]");
						}
						return tr;
					} else {
						if (type == "footer") {
							return (_115 == 1 ? dc.footer1 : dc.footer2)
									.find("tr[node-id=\"" + id + "\"]");
						} else {
							if (type == "selected") {
								return (_115 == 1 ? dc.body1 : dc.body2)
